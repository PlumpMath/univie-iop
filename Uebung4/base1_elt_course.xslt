...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="course">
        ...
        <xsl:for-each select="group">
            ...
            <xsl:value-of select="concat('    :gr', $groupid, ' a s:Group ;')"/> 
  rdfs:subClassOf <xsl:value-of select="concat(':id', $courseid)"/> ; 
  ...
  s:ap<xsl:value-of select="concat($groupid, $underscore, position())" /> a s:Appointment ;
    rdfs:subClassOf <xsl:value-of select="concat(':gr', $groupid)"/> ;
